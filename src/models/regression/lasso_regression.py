"""
Lasso regression model for stress score prediction.
"""
import pandas as pd
import numpy as np
...
class LassoRegressionModel:
    ...
    def __init__(self):
        ...
        self.model_config = self.config.get("models", {}).get("regression", {}).get("lasso", {})
        self.model = Lasso(**self.model_config)
        self.feature_columns = None
        self.target_column = 'weighted_stress_score'
        model_logger.info("LassoRegressionModel initialized")
    ...
    def train(self, df: pd.DataFrame, tune_hyperparams: bool = False) -> dict:
        ...
        """
        model_logger.info("Training lasso regression model")
        ...
        non_zero_coefs = np.sum(self.model.coef_ != 0)
        ...
        model_logger.info(f"Lasso regression trained: MSE={mse:.4f}, R2={r2:.4f}, Non-zero features={non_zero_coefs}")
        return results
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        results = df.copy()
        results['lasso_regression_prediction'] = predictions
        results['lasso_regression_residual'] = results.get(
